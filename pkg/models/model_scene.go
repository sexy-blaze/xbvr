...
type ResponseSceneList struct {
	...
	CountNotAvailable  int     `json:"count_not_available"`
...
func QueryScenes(r RequestSceneList, enablePreload bool) ResponseSceneList {
	...
	tx.Group("scenes.scene_id").Where("is_available = ?", true).Where("is_accessible = ?", false).Where("is_hidden = ?", false).Count(&out.CountNotAvailable)
